...
public class calculationtwo extends AppCompatActivity{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_calculationtwo);
...
//
    ...
    private void exqButton() {
        try {
            ...
            int dfals=getIntent().getIntExtra("a_5",0);
            int avals = 0;
            Y = 1;
            if (avals == 0) {
                for (i = 1; i <= bvals; i++) {
                    Vo = (float) ((pow((1 + xvals), i) - 1) / (pow((1 + xvals), (dfals+i)) * xvals));
                    V = (float) ((pow((1 + xvals), dfals) - 1) / (pow((1 + xvals), (dfals-1)) * xvals*dfals));
//                    Vo=(pow((1+r),i)-1)/(pow((1+r),(m+i))*r);
//                    Vi=(pow((1+r),m)-1)/(pow((1+r),(m-1))*m*r);
                    I=zvals*Vo/(V-Vo/i);
                    C=1-zvals*(Vo/(i*(V-Vo/i))+1/(1-yvals));
                    if (C > 0) {
            ...
            Toast.makeText(calculationtwo.this, "Error: Check all the values that are entered.", Toast.LENGTH_LONG).show();
        }
    }
